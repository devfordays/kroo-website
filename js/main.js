;(function () {
	...
	//listen for when mailchimp success message appears
	// select the target node
	var target = document.getElementById('mce-success-response');

	// create an observer instance
	var observer = new MutationObserver(function(mutations) {
		const thisObserver = this;
	  mutations.forEach(function(mutation) {
			$("#mce-success-response").addClass('form-submit-success');
			thisObserver.disconnect()
			$( "#mce-success-response" ).fadeOut( 3500 , function() {
				$("#mce-success-response").removeClass('form-submit-success');
				thisObserver.observe(target, config);
...
var observer2 = new MutationObserver(function(mutations) {
	const thisObserver = this;
	mutations.forEach(function(mutation) {
		$("#mce-error-response").addClass('form-submit-failure'); //hooked up to google tag manager to observe when form fails
		thisObserver.disconnect() //stop observing to prevent infinite loop
		setTimeout(function(){
			$("#mce-error-response").removeClass('form-submit-failure'); // to allow google tag manager to watch for multiple errors
			thisObserver.observe(target2, config);
		},1000);
	});
});
...
// if open register modal,
//1 - take form from bottom
//2 - insert into form content and add class of modal to form and form submit button
//if close register modal, take form from "inside" modal and insert into form content

$('#registerModal').on('show.bs.modal',function(event){
	resetWaitlistForm();
	$('#mc_embed_signup').appendTo('div.modal-body');
	$('input#mc-embedded-subscribe').removeClass('onPageSubmitBtn').addClass('modalSubmitBtn');
	$('form#mc-embedded-subscribe-form').removeClass('onPageWaitlistForm').addClass('modalWaitlistForm');
});

$('#registerModal').on('hidden.bs.modal', function(event){
	resetWaitlistForm();
	$('#mc_embed_signup').appendTo('#mc_embed_on_page_wrapper');
	$('input#mc-embedded-subscribe').removeClass('modalSubmitBtn').addClass('onPageSubmitBtn');
	$('form#mc-embedded-subscribe-form').addClass('onPageWaitlistForm').removeClass('modalWaitlistForm');
});

const resetWaitlistForm = function(){
	$('#mce-error-response').empty().css("display", "none");
	$('form#mc-embedded-subscribe-form input[type="text"], form#mc-embedded-subscribe-form input[type="email"] ').val('');
	$('form#mc-embedded-subscribe-form select').val('');
}
	...
	var toggleMobileClasses = function(){
		...
		$("#google-play-download-button").click(function(){
			$("#mc-embedded-subscribe").removeClass('ios-waitlist-submit-button').addClass('android-waitlist-submit-button');
			$("#mc-embedded-subscribe-form").removeClass('ios-waitlist-form').addClass('android-waitlist-form');
		});

		$("#ios-download-button").click(function(){
			$("#mc-embedded-subscribe").removeClass('android-waitlist-submit-button').addClass('ios-waitlist-submit-button');
			$("#mc-embedded-subscribe-form").removeClass('android-waitlist-form').addClass('ios-waitlist-form');
		});

	}

	var feeWayPoint = function(){
		if ( $('#fh5co-fee').length > 0 ) {
			$('#fh5co-fee').waypoint( function( direction ) {
				...
				if( direction === 'down' && !$(this).hasClass('animated') ) {
					...
					setTimeout(function(){
						...
						new Chart(document.getElementById("feeChart"),{
							type:"bar",
							data:{
								labels:["Visa","Mastercard","American Express","Travelex", "MUNNY"],
								datasets:[
									{
									data:[3,3,2.7,15,1],
									fill:true,
									backgroundColor :["#FFB9A9","#FFB9A9","#FFB9A9","#FFB9A9","#FFFFFF"],
									}
							]},
							options:{
								events: [],
								...
								title:{
									display: true,
									text : "Typical Currency Conversion Fees - %",
									fontSize: 16,
									fontColor:"#ffffff",
									fontWeight:500,
								},
